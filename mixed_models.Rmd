...
pData(annot) <- dat
saveRDS(annot, file="data/sample_phenotype_pcs.rds")
```
...
```{r null_model_fit_mm}
keep <- annot$sample.id %in% colnames(kinship)
nullmod <- fitNullModel(annot[keep,], outcome="height", 
                        covars=c("sex", "age", "study", paste0("PC", 1:5)), 
                        cov.mat=kinship, group.var="study", verbose=FALSE)
saveRDS(nullmod, file="data/null_mixed_model.rds")
```
